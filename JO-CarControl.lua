...
RegisterCommand("carmenu", function(source)
    TriggerEvent('ccvehmenu:client:openMenu') 
end)
...
AddEventHandler('ccvehmenu:client:openMenu', function()
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        QBCore.Functions.Notify("Enter Vehicle", "error", 3500)
    end
end)
...
RegisterNetEvent('qb-vehiclemenu:seatcontrol', function(args)
    ...
    local IsSeatFree = IsVehicleSeatFree(veh, -2)
        ...
        if veh == false then
            QBCore.Functions.Notify('Enter Vehicle')
        end
end)
...
RegisterNetEvent('qb-vehiclemenu:enginecontrol:on', function(args)
    ...
    if IsSeatFree == false then
        ...
        QBCore.Functions.Notify("Swap Seats First", "error", 3500)
        TriggerEvent('ccvehmenu:client:engineMenu')
    end
end)
...
RegisterNetEvent('qb-vehiclemenu:neoncontrol', function()
    ...
    if not hasNeons then 
        QBCore.Functions.Notify('Install Neons', 'error')
